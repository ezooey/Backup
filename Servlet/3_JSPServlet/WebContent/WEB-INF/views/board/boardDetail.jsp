		...
		<div class="tableArea">
			<form action="<%= request.getContextPath() %>/boardUpdateForm.bo" id="detailForm" method="post">
				<table>
					...
					<tr>
						<td colspan="6">
							<textarea cols="60" rows="15" style="resize:none;" readonly>
								<%= b.getBoardContent() %>
							</textarea>
							<input type="hidden" name="content" value="<%= b.getBoardContent() %>">
							<input type="hidden" size="50" name="bId" value="<%= b.getBoardId() %>">
						</td>
					</tr>
				</table>
		...
		$('#addReply').click(function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					$('#replyContent').val(''); // 댓글 쓰고나면 작성칸에 쓴 내용 사라지게 함
				},
				error: function(data){
					console.log(data);
				}
